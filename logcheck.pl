...
while ( $count <= $#ARGV ) {
  $a = lc( $ARGV[$count] );
  for ($a) {
    /^-?3(d)?$/ && do { $try3d = 1; $try4d = 0; $count++; next; };
    /^-?4(d)?$/ && do { $try3d = 0; $try4d = 1; $count++; next; };
...
sub walk3d {
  ...
  open( A, "c:/games/inform/threediopolis.inform/Source/story.ni" )
    || open( A, "story.ni" )
    || die("No story.ni");
  ...
  open( A, "c:/games/inform/threediopolis.inform/Source/logic.txt" );
  ...
  print
    "TEST RESULTS:threediopolis-walkthrough,$permissible,$blammo,$success,\n";
...
sub walk4d {
  ...
  open( A, "c:/games/inform/fourdiopolis.inform/source/fourdiopolis-logic.txt" )
    || die("No 4dop logic file.");
  ...
  while ( $a = <A> ) {
    ...
    if ( ( $a =~ /,/ ) && ( $a !~ /\./ ) ) {
      ...
      $a =~ s/\([^\)]*\)//g;
      $a =~ s/[ \*]//g;
      if   ($currentSummary) { $currentSummary = "$currentSummary,$a"; }
      else                   { $currentSummary = $a; $lastTableStart = $. }
      my @check = split( /,/, $a );
      if ( $#check != 4 ) {
        print "WARNING need 5 elements per line at line $.: $a\n";
        $blammo++;
      }
      next;
    }
...
sub setTable {
  ...
  while ( $b = <B> ) {
    ...
    if ($foundTable) {
      if ( $b !~ /[a-z]/ ) {
        $foundTable = 0;
        if ($thisRound) { print "$thisRound errors found in $lastTab.\n"; }
  ...
  if ( !$everFound ) {
    print("$tabname was not found. Check your logic file and/or source.\n");
  }
  else {
...
sub verify4 {
  ...
  for ( 0 .. $#cs ) {
    ...
    my $temp = lc( $cs[$_] );
    my $q    = myloc( $locs[$_] );
    if ( ( $temp eq $locs[$_] ) || ( lc($temp) eq lc($q) ) ) { next; }
    else {
      print
"Mismatch: $temp != $q and $temp != $locs[$_] at line $curLine in $curTab.\n";
      $blammo++;
      $thisRound++;
      $lineErr = 1;
      last;
  ...
  for $q ( sort keys %loc ) {
    if ( $q =~ /^$index/i ) {
      if ( $ents[$count] =~ /\?/ ) {
        print "$q/$ents[$count] is partially solved.\n";
        next;
      }
...
sub verify {
  ...
  for $q ( sort keys %loc ) {
    ...
    if (
      ( $q =~ /^$index/i )
      || ( ( lc($index) eq "friends" ) && ( $friends{ lc($q) } )
        || ( !$friends{ lc($q) } && $index =~ /[0-9]/ ) )
      )
    {
...
3(d) = run 3d only
4(d) = run 4d only
a = show all (how unsolved headers should look)
p = permissible errors (default is 0)
v = 3d verbose
